...
export class CmsService {
  ...
  async EditBlog(
    ...
    const newblog =  new BlogPostEntity()
    newblog.blogPost = dto.blog,
  ...
  //deleteblogpost
  async deleteBlogPost(id:number, userid:number):Promise<{msg:string}>{
    const blogger = await this.userrepo.findOne({where:{id:userid}})
    ...
    return {msg:'the blogpost has been deleted '}
  }
