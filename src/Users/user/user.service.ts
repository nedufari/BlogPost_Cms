...
export class UserService{
    constructor(@InjectRepository(UserEntity)private readonly adminrepo:UserRepository,
 ...
 public  async signToken(id: number, email: string,role:string) {
    ...
    async createUser(
        ...
        const emiailverificationcode =  await this.generateEmailToken()
         ...
         // mail
         await this.mailerservice.SendVerificationeMail(dto.email, emiailverificationcode,dto.fullname)
    ...
    async AdminResendemailVerificationLink (dto:RequestOtpResendDto):Promise<{message:string}>{
        ...
        //save the token
         ...
         await this.noticicationrepo.save(notification)
           ...
           //send mail 
           await this.mailerservice.SendVerificationeMail(newOtp.email,emiailverificationcode, emailexsist.fullname)
